...
{  
    ...
    {
        private const string cKey_SmtpServer = "Server Smtp";
        private const string cKey_SmtpPort = "Port Smtp";
        private const string cKey_SmtpUsername = "Utilisateur Smtp";
        private const string cKey_SmtpPassword = "Mot de passe Smtp";
        private const string cKey_SmtpEMailFrom = "Expediteur Smtp";
    ...
    public class ApplicationUserManager : UserManager<ApplicationUser, int>    
    {
        ...
        public static ApplicationUserManager Create(IdentityFactoryOptions<ApplicationUserManager> options, IOwinContext context)
        {
            ...
            manager.RegisterTwoFactorProvider("Code de sécurité", new PhoneNumberTokenProvider<ApplicationUser, int>
            {
                MessageFormat = "Votre code de securité est {0}"
            });

            manager.RegisterTwoFactorProvider("Email Code", new EmailTokenProvider<ApplicationUser, int>
            {
                Subject = "Code de securité",
                BodyFormat = "Votre code de securité est {0}"
            });
        ...
        public static ApplicationUser GetUserByName(string _userName)
        {
            return GetUserByName(new RBACDbContext(), _userName);
        }

        public static ApplicationUser GetUserByName(RBACDbContext db, string _userName)
        {
            ApplicationUser _retVal = null;
            try
            {
                _retVal = db.Users.Where(p => p.UserName == _userName).Include("Roles").Include(x => x.Roles.Select(r => r.Role.PERMISSIONS)).FirstOrDefault();
            }
            catch (Exception)
            {
            }
        ...
        public static bool UpdateRole(RoleViewModel _modifiedRole)
        {
            ...
            try
            {
                using (RBACDbContext db = new RBACDbContext())
                {
                    ...
                    db.Entry(_role2Modify).Entity.Name = _modifiedRole.RoleName;
